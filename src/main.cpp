...
void loop(void)
{
    if (push_done(IBUPROFEN)) {
        ...
        do_publish("vending-machine", "Ibuprofen");
        // Buy and do stuff
    }
    if (push_done(DIPHEMHYDRAMINE)) {

        do_publish("vending-machine", "Diphemhydramine");
        // Buy and do stuff
    }
    if (push_done(PARACETAMOL)) {

        do_publish("vending-machine", "Paracetamol");
        // Buy and do stuff
    }
    if (push_done(ASPIRIN)) {

        do_publish("vending-machine", "Aspirin");
        // Buy and do stuff
    }
    if (push_done(CREDIT)) {
